...
export default function Nav() {
  ...
  const roleBadge = isAdmin
    ? <span className="ml-1 text-[9px] bg-bamboo/10 text-bamboo px-1.5 py-0.5 rounded-full uppercase tracking-widest">admin</span>
    : <span className="ml-1 text-[9px] bg-slate/10 text-slate px-1.5 py-0.5 rounded-full uppercase tracking-widest">reader</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center">
          ...
          <div className="hidden sm:flex items-center gap-3 ml-auto py-2">
            ...
            <button data-testid="logout-btn" onClick={logout}
              className="flex items-center gap-1 text-xs text-stone hover:text-red-500 transition-colors" title="Sign out">
              <LogOut size={13} />
            </button>
          </div>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
              ...
              <div className="px-6 py-5 border-b border-fog-dark bg-bamboo-deeper">
                ...
                <p className="text-white/40 text-xs mt-0.5">
                  {user?.display_name || user?.username} · {isAdmin ? 'Admin' : 'Reader'}
                </p>
              </div>
